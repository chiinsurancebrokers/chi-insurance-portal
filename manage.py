...
def list_policies():
    ...
    for policy in policies:
        ...
        print(f"License Plate: {policy.license_plate or 'N/A'}")
    ...
    if not clients:
        ...
        for client in clients:
            ...
            for policy in client.policies:
                print(f"  - Policy ID {policy.id}: {policy.policy_type} | {policy.provider} | Expires: {policy.expiration_date}")
    print("=" * 60)
    session.close()
...
def list_pending():
    ...
    for payment in payments:
        ...
        print(f"\n{client.name}")
        print(f"  Policy ID: {policy.id} | Policy #: {policy.policy_number or 'N/A'}")
        print(f"  Type: {policy.policy_type}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("  python manage.py delete <client_id>")
        print("  python manage.py delete-policy <policy_id>")
