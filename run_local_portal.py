...
def parse_csv_changes(filepath, agent='3p'):
    ...
    for line_num, line in enumerate(lines[header_idx + 1:], start=1):
        ...
        def normalize_name(name):
            """Normalize Greek client names for matching"""
            import re
            n = name.upper().strip()
        ...
        # If not found, try normalized match against all clients
        if not client:
        ...
        # For BU: Check if this is an amendment (Πρόσθετη Πράξη)
        category = row.get('Κατηγορία', '').strip()
        is_amendment = 'Πρόσθετη' in category or 'ΠΡΟΣΘΕΤΗ' in category.upper()
        
        if not client:
            # Skip amendments for clients that don't exist (shouldn't happen)
            if is_amendment:
                print(f'WARNING: Amendment for unknown client {client_name} - skipping')
                continue
        ...
        # Fallback: check by license plate
        ...
        # Fallback: check by type/provider for this client
        if not existing:
            existing = db_session.query(Policy).filter_by(
                client_id=client.id,
                policy_type=policy_type,
        ...
        # Handle amendments - UPDATE existing policy instead of creating new
        if is_amendment and existing:
            # Amendment: adjust premium on existing policy
            new_premium = existing.premium + premium  # premium can be negative for refunds
            if existing.premium != new_premium:
                changes['updated_policies'].append({
                    ...
                    'new_start': existing.start_date,  # Keep original start date
                    'old_expiry': existing.expiration_date,
                    'new_expiry': existing.expiration_date,  # Keep original expiry
                    'is_amendment': True,
                    'amendment_type': category,
                    'amendment_amount': premium
                })
            continue
