...
class ThreePInsuranceExtractor:
    ...
    def save_to_csv(self, data_list, output_path):
        ...
        preferred_order = [
            'name', 'email', 'phone', 'license_plate', 'policy_type',
            'provider', 'policy_number', 'premium', 'due_date',
            'coverage_start', 'coverage_end', 'vehicle_model', 'vehicle_year',
            'address', 'postal_code', 'city', 'tax_id', 'payment_code',
            'source_file', 'extracted_date'
        ]
...
def main():
    ...
    extractor = ThreePInsuranceExtractor()
    
    print("=" * 60)
    print("3P INSURANCE PDF EXTRACTOR")
    print("=" * 60)
